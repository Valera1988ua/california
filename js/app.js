 ...
 */
    ...
    (() => {
        ...
        let helpFunctions_bodyLockStatus = true;
        let helpFunctions_bodyLockToggle = (delay = 500) => {
            if (document.documentElement.classList.contains("lock")) bodyUnlock(delay); else helpFunctions_bodyLock(delay);
        };
        let bodyUnlock = (delay = 500) => {
            let body = document.querySelector("body");
            if (helpFunctions_bodyLockStatus) {
                ...
                helpFunctions_bodyLockStatus = false;
                setTimeout((() => {
                    helpFunctions_bodyLockStatus = true;
                }), delay);
            }
        };
        let helpFunctions_bodyLock = (delay = 500) => {
            let body = document.querySelector("body");
            if (helpFunctions_bodyLockStatus) {
                let lock_padding = document.querySelectorAll("[data-lp]");
                for (let index = 0; index < lock_padding.length; index++) {
        ...
        let helpFunctions_slideUp = (target, duration = 500, showmore = 0) => {
        ...
        let helpFunctions_slideDown = (target, duration = 500, showmore = 0) => {
            if (!target.classList.contains("_slide")) {
                target.classList.add("_slide");
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (e => {
                if (helpFunctions_bodyLockStatus && e.target.closest(".icon-menu")) {
                    helpFunctions_bodyLockToggle();
                    document.documentElement.classList.toggle("menu-open");
                }
            }));
        }
        function menuBurger_menuClose() {
            bodyUnlock();
        ...
        class Spoller {
            ...
            open(el) {
                ...
                helpFunctions_slideDown(el.nextElementSibling, this.options.speed);
            }
            close(el) {
                el.classList.remove("_spoller-active");
                helpFunctions_slideUp(el.nextElementSibling, this.options.speed);
            }
        ...
        modules_projectModules.spoller = new Spoller("[data-spollers]", {
            speed: 300,
            oneOpen: true,
            allActive: false,
            spollerActive: "3",
        ...
        let gotoblock_gotoBlock = (targetBlock, noHeader = false, speed = 500, offsetTop = 0) => {
            ...
            if (targetBlockElement) {
                ...
                document.documentElement.classList.contains("menu-open") ? menuBurger_menuClose() : null;
        ...
        const inputMasks = document.querySelector('input[type="tel"]');
        new Inputmask("+3(999)999-99-99");
        const maskPhone = () => {
            const phone = inputMasks.inputmask.unmaskedvalue();
            return 10 === phone.length;
        };
        function formSubmit() {
            ...
            async function formSubmitAction(form, event) {
            ...
            function formSent(form, responseResult = ``) {
                ...
                setTimeout((() => {
                    if (modules_projectModules.popup) {
                        const popup = form.dataset.popupMessage;
                        popup ? modules_projectModules.popup.open(popup) : null;
                    }
                }), 0);
                formValidate.formClean(form);
        ...
        let formValidate = {
            ...
            validateInput(formRequiredItem) {
                ...
                if ("email" === formRequiredItem.dataset.required) {
                    ...
                    this.addError(formRequiredItem);
                    error++;
                } else if ("name" === formRequiredItem.dataset.required && formRequiredItem.value.length < 3) {
                    this.addError(formRequiredItem);
                    error++;
                } else if ("phone" === formRequiredItem.dataset.required && false == maskPhone()) {
                    this.addError(formRequiredItem);
                    error++;
            ...
            formClean(form) {
                ...
                setTimeout((() => {
                    ...
                    if (modules_projectModules.select) {
                        let selects = form.querySelectorAll(".select");
                        if (selects.length) for (let index = 0; index < selects.length; index++) {
                            const select = selects[index].querySelector("select");
                            modules_projectModules.select.selectBuild(select);
                        }
                    }
                }), 0);
            },
        ...
        function formFieldsInit(options = {
            ...
            if (options.viewPass) document.addEventListener("click", (event => {
        ...
        function Keyboard({swiper, extendParams, on, emit}) {
            ...
            function handle(event) {
                ...
                if (swiper.params.keyboard.onlyInViewport && (isPageUp || isPageDown || isArrowLeft || isArrowRight || isArrowUp || isArrowDown)) {
                    let inView = false;
                    if (utils_elementParents(swiper.el, `.${swiper.params.slideClass}, swiper-slide`).length > 0 && 0 === utils_elementParents(swiper.el, `.${swiper.params.slideActiveClass}`).length) return;
                    const el = swiper.el;
        ...
        function searchInput() {
            ...
            const searchClose = inputSearch.querySelector(".input-search__close");
            const iconSearchBlock = document.querySelector(".action-header");
            const searchIcon = iconSearchBlock.querySelector(".action-header__search");
            searchIcon.addEventListener("click", (e => {
                const el = e.target;
                if (el.closest(".action-header")) inputSearch.classList.add("_search-active");
            }));
            searchClose.addEventListener("click", (e => {
                const el = e.target;
                if (el.closest(".input-search") || inputSearch.classList.contains("_search-active")) inputSearch.classList.remove("_search-active");
            }));
            document.addEventListener("click", (e => {
                const el = e.target;
                if (inputSearch.classList.contains("_search-active")) if (!el.closest(".input-search") && !el.closest(".action-header")) inputSearch.classList.remove("_search-active");
            }));
        }
        searchInput();
        function productCart() {
            const products = document.querySelectorAll(".item-product");
            if (!isMobile.any()) if (products) products.forEach((el => {
                let currentProduct = el;
                const imageSwitchItems = currentProduct.querySelectorAll(".image-switch__item");
                const imagePagination = currentProduct.querySelector(".pagination-image");
                if (imageSwitchItems.length > 1) imageSwitchItems.forEach(((el, index) => {
                    el.setAttribute("data-index", index);
                    imagePagination.innerHTML += `<li class="pagination-image__item ${0 == index ? "_active" : ""}" data-index="${index}"></li>`;
                    el.addEventListener("mousemove", (e => {
                        e.preventDefault();
                        currentProduct.querySelectorAll(".pagination-image__item").forEach((el => el.classList.remove("_active")));
                        currentProduct.querySelector(`.pagination-image__item[data-index="${e.currentTarget.dataset.index}"]`).classList.add("_active");
                    }));
                    el.addEventListener("mouseleave", (e => {
                        e.preventDefault();
                        currentProduct.querySelectorAll(".pagination-image__item").forEach((el => el.classList.remove("_active")));
                        currentProduct.querySelector(`.pagination-image__item[data-index="0"]`).classList.add("_active");
                    }));
                }));
            }));
        }
